...
const AlertDetailPanel: React.FC<AlertDetailPanelProps> = ({ alert }) => {
  ...
  useEffect(() => {
    // Reset anomaly data to null when the alert prop changes.
    // This ensures the loading skeleton is displayed while new data is fetched.
    setAnomalyData(null); 
  ...
  // Render a detailed skeleton while fetching the anomaly data.
  if (!anomalyData) {
    return (
        <div className="bg-navy-light rounded-2xl shadow-lg p-6 animate-pulse space-y-4">
            <div className="h-8 bg-navy-dark rounded w-3/4"></div>
            <div className="space-y-2">
                <div className="h-4 bg-navy-dark rounded w-1/4"></div>
                <div className="h-4 bg-navy-dark rounded w-3/4"></div>
            </div>
            <div className="space-y-2">
                <div className="h-4 bg-navy-dark rounded w-1/4"></div>
                <div className="h-4 bg-navy-dark rounded w-2/3"></div>
            </div>
            <div className="mt-4 bg-command-blue rounded-lg p-4 space-y-4">
                <div className="h-6 bg-navy-dark rounded w-1/2"></div>
                <div className="h-3 bg-navy-dark rounded w-full"></div>
                <div className="h-8 bg-navy-dark rounded w-full"></div>
            </div>
        </div>
    );
  }
      ...
      <div className="bg-command-blue rounded-lg p-4 space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <span className={`text-lg font-bold ${MLAnomalyService.getConfidenceColor(anomalyData.confidence)}`}>
        ...
        {anomalyData.is_anomaly && (
          <div className="flex items-center justify-between py-2 px-3 bg-navy-light rounded-md">
            ...
            <span className={`px-3 py-1 rounded font-bold text-sm ${MLAnomalyService.getPriorityColor(anomalyData.priority)} text-command-blue`}>
